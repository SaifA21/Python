...
def login():
    ...
    time.sleep(1)
    print("")
    showPass = decrypt(encryptLogin)
    print("THE PASSWORD IS:", showPass)
...
def forgotLogin(viewPass):
    ...
    while forgotCounter < 3:
        try:
            showSec = decrypt(encryptSecAnsEncrypted)
            print("The Car Make Is:", showSec)
...
def delPass (delNames,delPasswords,delNameTxt,delPassTxt,delChoice):
    ...
    print("These are the Password names:", delNames)
...
def editPass (passTxt, nameValues, passValues, choice):
    ...
    print("These are the Password names:", nameValues)
